...
from video_creation import (
    create_video_gradio_interface, get_available_music_files
)
...
def generate_video_for_music(filename):
    ...
    try:
        ...
        if not video_response or video_response.get('code') != 200:
            ...
            if (video_response.get('code') == 409 and
                    "Mp4 record already exists" in error_msg):
                print("🔧 VIDEO DEBUG: Video already exists, "
                      "retrieving existing video...")

                # Get the taskId from the error response
                existing_taskId = video_response.get('data', {}).get('taskId')
                if not existing_taskId:
                    return "Video exists but no taskId provided in API resp"

                print("🔧 VIDEO DEBUG: Existing video taskId: {}".format(
                    existing_taskId))
                ...
                # Download the existing video using the same filename as MP3
                    ...
                    return_msg += ("ℹ️ This video was already generated "
                                   "by the API")

                    return return_msg

                except Exception as download_error:
                    print("🔧 VIDEO DEBUG: Error downloading existing video: "
                          "{}".format(str(download_error)))
                    return ("Video exists but failed to download: "
                            "{}".format(str(download_error)))
        ...
        print("🔧 VIDEO DEBUG: Video generation completed. URL: "
              "{}".format(video_url))
...
def create_gradio_interface():
    ...
    with gr.Blocks(title="Suno Music Generator") as demo:
        ...
        with gr.Tab("Create Video with Lyrics"):
            gr.Markdown("## 🎬 Create Custom Video with Lyrics")
            gr.Markdown("*Create videos with background images and "
                       "timestamped lyrics overlay*")
            gr.Markdown("*Fetches lyrics from Suno API and creates "
                       "local MP4 videos*")

            with gr.Row():
                # Left column - Controls (1/3 width)
                with gr.Column(scale=1):
                    refresh_video_music_btn = gr.Button(
                        "🔄 Refresh Music List",
                        variant="secondary"
                    )

                    video_music_dropdown = gr.Dropdown(
                        label="Select Music File",
                        choices=get_available_music_files(),
                        value=None,
                        info="Choose music file for video creation"
                    )

                    background_image_input = gr.File(
                        label="📸 Upload Background Image",
                        file_types=["image"],
                        type="filepath"
                    )

                    create_video_btn = gr.Button(
                        "🎬 Create Video with Lyrics",
                        variant="primary",
                        size="lg"
                    )

                    video_creation_output = gr.Textbox(
                ...
                # Right column - Video Player (2/3 width)
                with gr.Column(scale=2):
                    video_player = gr.Video(
                        label="🎥 Created Video Preview",
                        value=None,
                        interactive=False,
                        height=600,
            ...
            # Refresh function for video creation tab
            def refresh_video_music_list():
                choices = get_available_music_files()
                return gr.Dropdown(
                    choices=choices,
                    value=None if choices else None
            ...
            # Event handlers
            refresh_video_music_btn.click(
                fn=refresh_video_music_list,
                outputs=video_music_dropdown
            )

            create_video_btn.click(
                fn=create_video_gradio_interface,
                inputs=[video_music_dropdown, background_image_input],
                outputs=[video_creation_output, video_player]
            )
